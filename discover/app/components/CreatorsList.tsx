...
export default function CreatorsList({
  ...
  const toggleDropdown = (e: React.MouseEvent, channelId: string) => {
    e.preventDefault();
    e.stopPropagation();
    ...
    >
      {safeCreators.map((creator) => (
        <Link
          ...
          className={
            isGrid
              ? `relative group flex flex-col items-center text-center p-5 bg-surface border border-border-subtle rounded-2xl transition-all duration-200 ease-in-out hover:bg-surface-raised hover:-translate-y-1 hover:shadow-md hover:border-brand active:scale-[0.99] w-full min-w-0 ${
                  activeDropdownChannelId === creator.channel_id ? "z-30" : ""
                }`
              : `relative group flex items-center gap-5 sm:gap-6 p-5 sm:p-6 bg-surface border border-border-subtle rounded-2xl transition-all duration-200 ease-in-out hover:bg-surface-raised hover:-translate-y-0.5 hover:shadow-md hover:border-brand active:scale-[0.99] ${
                  activeDropdownChannelId === creator.channel_id ? "z-30" : ""
                }`
          }
        >
          ...
          <div
            className={`absolute z-10 ${
              isGrid ? "top-3 right-3" : "top-1/2 -translate-y-1/2 right-12 sm:right-16"
            }`}
            onClick={(e) => {
              e.preventDefault();
              e.stopPropagation();
          ...
          >
            <button
              onClick={(e) => toggleDropdown(e, creator.channel_id)}
              className="flex items-center justify-center w-8 h-8 rounded-full text-secondary hover:text-primary hover:bg-surface-overlay transition-all duration-150"
            ...
            {/* Dropdown Menu */}
            {activeDropdownChannelId === creator.channel_id && (
              <div
                className="absolute right-0 mt-1 w-48 bg-surface border border-border rounded-md shadow-lg py-1.5 z-20 animate-fade-in"
                onClick={(e) => {
                  e.preventDefault();
                  e.stopPropagation();
                }}
              >
                <button
                  onClick={(e) => {
                    ...
                    e.stopPropagation();
                    setActiveDropdownChannelId(null);
                    if (onManageLists) onManageLists(creator);
                  }}
                  className="w-full text-left px-4 py-2 text-sm text-primary hover:bg-surface-raised flex items-center gap-2.5 transition-colors duration-150"
                >
