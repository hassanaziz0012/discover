...
from youtube.creators import get_creators, add_creators_metadata
...
async def bulk_add_creators(request: BulkAddRequest, db: Session = Depends(get_db)):
    """
    Bulk-add YouTube creators by fetching their channel metadata and persisting them to PostgreSQL.
    Does not fetch their videos; videos can be fetched later via channel refresh.
    """
    ...
    # Fetch and persist channel metadata for new creators without fetching videos
    added_list, errors = await asyncio.to_thread(
        add_creators_metadata, api_key, new_channel_ids, db
    )
